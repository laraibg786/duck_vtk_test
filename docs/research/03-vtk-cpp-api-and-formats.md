...
| `vtkCGNSReader` | `VTK::IOCGNS` | `vtkMultiBlockDataSet` | Reads binary CGNS files, low-level dependency on the CGNS library (present as a brew dependency: `cgns`). Supports structured and unstructured CGNS meshes, node/cell/face-centered data. Has `CanReadFile(const char*)`. |
