...
- DuckDB core defaults `CMAKE_CXX_STANDARD` to 11; our CMakeLists sets 17 as a CACHE var. Because DuckDB is the top-level project, ordering decides the winner, so the `Makefile` forces `-DCMAKE_CXX_STANDARD=17` on the command line. Verify: `grep CMAKE_CXX_STANDARD build/release/CMakeCache.txt`.
